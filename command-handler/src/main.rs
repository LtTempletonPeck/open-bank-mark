...
fn handle_cac(
    ...
    let producer_data = match cac.reason {
        None => ProducerData {
            topic: "account_creation_confirmed".to_string(),
            key,
            values: acc_vec(&values, cac),
        },
        Some(v) => ProducerData{
            topic: "account_creation_failed".to_string(),
            key,
...
fn handle_cmt(
    ...
    let key = uuid.hyphenated().to_string();
    {
        let producer_data = match cmt.reason {
            None => ProducerData{
                topic: "money_transfer_confirmed".to_string(),
                key,
                values: mtc_vec(values)
            },
            Some(v) => ProducerData{
                topic: "money_transfer_failed".to_string(),
                key,
                values: fail_vec(values, v)
            },
        };
...
fn send_bc(
    ...
    let producer_data = ProducerData{
        topic: "balance_changed".to_string(),
        key: iban_string,
...
fn send_loop(receiver: &Receiver<ProducerData>) {
    ...
    loop {
        ...
        producer.send(&producer_data.topic, Option::Some(&producer_data.key[..]), producer_data.values);
    }
}
